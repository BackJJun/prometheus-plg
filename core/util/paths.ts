...
// Load .env from project root by searching upward
...
const CONTINUE_GLOBAL_DIR = (() => {
  ...
  return path.join(os.homedir(), ".prometheus");
})();
...
export function getConfigYamlPath(ideType?: IdeType): string {
  ...
  if (!fs.existsSync(p) && !fs.existsSync(getConfigJsonPath())) {
    // Create simplified default config with server_api_url from .env
    const serverApiUrl =
      process.env.DEFAULT_SERVER_API_URL || "http://localhost:8000";
    const defaultYamlContent = `name: "prometheus"
version: "1.0.0"
server_api_url: "${serverApiUrl}"
`;
    fs.writeFileSync(p, defaultYamlContent);
