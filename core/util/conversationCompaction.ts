...
export async function generateConversationSummary({
  ...
  index,
  currentModel,
}: GenerateConversationSummaryParams): Promise<string> {
  const historyUpToIndex = history.slice(0, index + 1);
  ...
  filteredHistory.forEach((item) => {
    ...
    if (item.message.role === "assistant" && item.message.toolCalls) {
      item.message.toolCalls.forEach((toolCall) => {
        const hasToolResult = filteredHistory.find(
          (historyItem) =>
            historyItem.message.role === "tool" &&
            historyItem.message.toolCallId === toolCall.id,
        );
