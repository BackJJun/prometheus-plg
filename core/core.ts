...
import historyManagerDB from "./util/historyDB";
import modelManagerDB from "./util/modelDB";
...
export class Core {
  ...
  private async resolveModel(
    ...
    const config = (await this.configHandler.loadConfig()).config;

    console.log(
      "[DEBUG] resolveModel - config.serverApiUrl:",
      config?.serverApiUrl,
    );

    // 1. Try to use the model description passed from GUI (API model)
    if (modelDescription) {
      const ideSettings = await this.ide.getIdeSettings();
      const readFile = (filepath: string) => this.ide.readFile(filepath);
      const getUriFromPath = async (path: string) => undefined; // Not needed for API models

      console.log("[DEBUG] resolveModel - Passing to llmFromDescription:", {
    ...
    // 2. Fallback to config model (legacy behavior)
  ...
  private registerMessageHandlers(ideSettingsPromise: Promise<IdeSettings>) {
    ...
    on("history/list", async (msg) => {
      // Use DB-based history manager for server storage
      try {
        const accessToken = (msg.data as any).accessToken;
        if (accessToken) {
          historyManagerDB.setAccessToken(accessToken);
        }

        const dbSessions = await historyManagerDB.list(msg.data);
        ...
        const allSessions = [...dbSessions, ...remoteSessions].sort(
          (a, b) =>
        ...
        console.error("[Core] Failed to list sessions:", error.message);
        return [];
      }
    });
    ...
    on("history/delete", async (msg) => {
      ...
      try {
        ...
        await historyManagerDB.delete(msg.data.id);
      } catch (error: any) {
        console.error("[Core] Failed to delete session:", error.message);
        throw error;
      }
    });
    ...
    on("history/load", async (msg) => {
      ...
      try {
        ...
        return await historyManagerDB.load(msg.data.id);
      } catch (error: any) {
        console.error("[Core] Failed to load session:", error.message);
        throw error;
      }
    });

    on("history/loadLast", async (msg) => {
      // Load only from DB
      try {
        const accessToken = (msg.data as any).accessToken;
        if (accessToken) {
        ...
        return await historyManagerDB.loadLast(msg.data.workspaceDirectory);
      } catch (error: any) {
        console.error("[Core] Failed to load last session:", error.message);
        throw error;
      }
    });
    ...
    on("history/save", async (msg) => {
      // Save only to DB (no local fallback)
      try {
        // Get access token from the message data if available
        const accessToken = (msg.data as any).accessToken;
        if (accessToken) {
          historyManagerDB.setAccessToken(accessToken);
        }

        await historyManagerDB.save(msg.data);
      } catch (error: any) {
        console.error("[Core] Failed to save session:", error.message);
        throw error;
      }
    });
    ...
    // Models
    on("models/list", async (msg) => {
      try {
        const accessToken = (msg.data as any).accessToken;
        if (accessToken) {
          modelManagerDB.setAccessToken(accessToken);
        }

        return await modelManagerDB.list();
      } catch (error: any) {
        console.error("[Core] Failed to list models:", error.message);
        return [];
      }
    });
    ...
    on("config/updateSelectedModel", async (msg) => {
      ...
      // Removed config reload to prevent overwriting API models
      // await this.configHandler.reloadConfig(
    ...
    on("llm/complete", async (msg) => {
      const model = await this.resolveModel((msg.data as any).modelDescription);
    ...
    on("llm/compileChat", async (msg) => {
      const { messages, options, modelDescription } = msg.data as any; // Cast to any to avoid type error
      const model = await this.resolveModel(modelDescription);
    ...
    on("chatDescriber/describe", async (msg) => {
      const currentModel = await this.resolveModel(
        (msg.data as any).modelDescription,
      );
  ...
  private async handleToolCall(
    ...
    const llm = await this.resolveModel(modelDescription);
  ...
  private getContextItems = async (
    ...
    const { config } = await this.configHandler.loadConfig();
    if (!config) {
      console.log("[Core.getContextItems] No config loaded");
      return [];
    }

    const { name, query, fullInput, selectedCode } = msg.data;
    console.log("[Core.getContextItems] Received request:", {
      name,
      query: query?.substring(0, 100),
    });
    console.log(
      "[Core.getContextItems] Available providers:",
      config.contextProviders?.map((p) => p.description.title),
    );

    // LLM is optional for most context providers (e.g., FileContextProvider doesn't use it)
    // Only some providers like codebase search may need an LLM
    const llm = (await this.configHandler.loadConfig()).config
      ?.selectedModelByRole.chat;

    // Don't throw error if no LLM - many context providers don't need it
    if (!llm) {
      console.log(
        "[Core.getContextItems] No chat model selected, proceeding without LLM",
      );
    }
    ...
    if (!provider) {
      console.log("[Core.getContextItems] Provider not found:", name);
      return [];
    }
    console.log(
      "[Core.getContextItems] Found provider:",
    ...
    try {
      ...
      const items = await provider.getContextItems(query, {
        config,
        llm: llm as any, // May be undefined for providers that don't need it
