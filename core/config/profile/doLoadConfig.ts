...
export default async function doLoadConfig(options: {
  ...
  if (configLoadInterrupted || !newConfig) {
    ...
    newConfig = {
      ...
      rules: [],
      requestOptions: {},
      serverApiUrl: undefined, // Will cause error if used, forcing proper config
    };

    // Filter out fatal errors related to config loading
    errors = errors?.filter((e) => !e.fatal) || [];
  ...
  // Update DB API base URLs from config
  if (newConfig.serverApiUrl) {
    modelManagerDB.setApiBaseUrl(newConfig.serverApiUrl);
    historyManagerDB.setApiBaseUrl(newConfig.serverApiUrl);
    console.log(
      "[doLoadConfig] Updated DB API base URLs to:",
      newConfig.serverApiUrl,
    );
  } else {
    console.warn(
      "[doLoadConfig] No serverApiUrl in config, DB APIs will use default",
    );
  }
