...
export class ConfigHandler {
  ...
  async getLocalProfiles(options: LoadAssistantFilesOptions) {
    /**
     * Users can define local profile configs in `.prometheus/agents`,
     * `.prometheus/assistants`, or `.prometheus/configs`.
     */
  ...
  async openConfigProfile(
    ...
    if (!openProfileId) {
      await this.ide.openFile(localPathToUri(getPrimaryConfigFilePath()));
      return;
    }
    const profile = this.currentOrg?.profiles.find(
    ...
    if (profile.profileDescription.profileType === "local") {
      ...
      await this.ide.openFile(
        configFile.startsWith("file://")
          ? configFile
          : localPathToUri(configFile),
      );
    } else {
      const env = await getControlPlaneEnv(this.ide.getIdeSettings());
