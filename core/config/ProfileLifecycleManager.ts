...
export class ProfileLifecycleManager {
  ...
  async getSerializedConfig(
    ...
    if (this.savedBrowserConfigResult) {
      ...
      // Remove models from serialized config - models come from /api/models only
      // This prevents config.yaml from overwriting API-loaded models
      const configWithoutModels: BrowserSerializedContinueConfig = {
        ...serializedConfig,
        modelsByRole: {
          chat: [],
          apply: [],
