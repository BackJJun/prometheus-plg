...
const AGENT_SUBDIRS = ["agents", "assistants"];
...
export function isContinueConfigRelatedUri(uri: string): boolean {
  return (
    ...
    ((uri.includes(".prometheus") || uri.includes(".continue")) &&
      (uri.endsWith(".yaml") ||
        uri.endsWith(".yml") ||
        uri.endsWith(".json"))) ||
    [...BLOCK_TYPES, ...AGENT_SUBDIRS, "configs"].some(
      (blockType) =>
        uri.includes(`.prometheus/${blockType}`) ||
...
export function isContinueAgentConfigFile(uri: string): boolean {
  ...
  return (
    normalizedUri.includes(`/.prometheus/agents/`) ||
    normalizedUri.includes(`/.prometheus/assistants/`) ||
    normalizedUri.includes(`/.prometheus/configs/`) ||
...
export function getDotContinueSubDirs(
  ...
  // Workspace .prometheus/<subDirName>
  if (options.includeWorkspace) {
    fullDirs = workspaceDirs.flatMap((dir) => [
      joinPathsToUri(dir, ".prometheus", subDirName),
      joinPathsToUri(dir, ".continue", subDirName),
    ]);
  }

  // ~/.prometheus/<subDirName>, with ~/.continue/<subDirName> as compatibility fallback
  if (options.includeGlobal) {
    fullDirs.push(localPathToUri(getGlobalFolderWithName(subDirName)));
    fullDirs.push(
      localPathToUri(path.join(os.homedir(), ".continue", subDirName)),
    );
  }
...
/**
 * This method searches in both ~/.prometheus and workspace .prometheus.
 */
