...
async function loadConfigYaml(options: {
  ...
  if (config && isAssistantUnrolledNonNullable(config)) {
    ...
    // Warn if server_api_url is missing (non-fatal)
    if (!config.server_api_url) {
      errors.push({
        fatal: false,
        message:
          "server_api_url is missing in config.yaml. It's recommended to add 'server_api_url: <your-api-url>' to your config.yaml file.",
      });
    }
  }
...
export async function configYamlToContinueConfig(options: {
  ...
  const continueConfig: ContinueConfig = {
    ...
    serverApiUrl: config.server_api_url,
    securityTarget: config.security_target,
  };

  console.log(
    "[DEBUG] configYamlToContinueConfig - serverApiUrl:",
    continueConfig.serverApiUrl,
  );
  console.log(
    "[DEBUG] configYamlToContinueConfig - from config.server_api_url:",
