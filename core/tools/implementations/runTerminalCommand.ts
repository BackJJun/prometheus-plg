...
const DEFAULT_WINDOWS_OUTPUT_ENCODING = "cp949";

function hasDecodeIssues(text: string): boolean {
  return text.includes("\uFFFD");
}

function getWindowsOutputEncoding(): string {
  const configured = process.env.CONTINUE_TERMINAL_OUTPUT_ENCODING?.trim();
  if (configured && iconv.encodingExists(configured)) {
    return configured;
  }

  return DEFAULT_WINDOWS_OUTPUT_ENCODING;
}

// Decode terminal output according to platform-specific encodings.
function getDecodedOutput(data: Buffer): string {
  if (process.platform === "win32") {
    const utf8Output = iconv.decode(data, "utf8");
    if (!hasDecodeIssues(utf8Output)) {
      return utf8Output;
    }

    return iconv.decode(data, getWindowsOutputEncoding());
  } else {
...
function getShellCommand(command: string): { shell: string; args: string[] } {
  if (process.platform === "win32") {
    ...
    return {
      ...
      args: ["-NoLogo", "-ExecutionPolicy", "Bypass", "-Command", utf8Command],
    };
  } else {
