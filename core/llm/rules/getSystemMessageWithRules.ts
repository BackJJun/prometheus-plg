...
/**
 * Checks if a rule is a root-level rule (.prometheus directory or no file path)
 */
const isRootLevelRule = (rule: RuleWithSource): boolean => {
  return !rule.sourceFile || rule.sourceFile.includes(".prometheus/"); // sourceFile path is absolute - hence we need to check for it in between
};
