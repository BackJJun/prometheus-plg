  ...
  protected async *_streamChat(
    ...
    if (body.stream === false) {
      ...
      if (shouldDebugRawChatStream()) {
        console.log(
          "[RAW_CHAT_NON_STREAM_RESPONSE][openai]",
          JSON.stringify(data),
        );
      }
    ...
    for await (const value of streamSse(response)) {
      if (shouldDebugRawChatStream()) {
        console.log("[RAW_CHAT_STREAM_CHUNK][openai]", JSON.stringify(value));
      }
      const chunk = fromChatCompletionChunk(value);
      if (shouldDebugRawChatStream() && chunk) {
        console.log(
          "[PARSED_CHAT_CHUNK][openai]",
          JSON.stringify({
            role: chunk.role,
            contentPreview:
