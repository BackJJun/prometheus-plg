...
const BRIEF_LAZY_INSTRUCTIONS = `For larger codeblocks (>20 lines) shown for DEMONSTRATION ONLY, you may use brief placeholders for unmodified sections, e.g. '// ... existing code ...'. However, when using edit tools (edit_existing_file), you MUST output the COMPLETE file content - placeholder comments will cause code loss.`;
...
<important_rules>
  ...
  When the user asks to create or modify files, use edit tools to make the changes directly in files instead of only returning code snippets.
  If the user message includes a marker like [REFERENCE_SELECTED id=... name=...], use the read_reference tool with that doc_id to load the reference content before implementing.
