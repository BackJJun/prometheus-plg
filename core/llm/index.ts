...
export abstract class BaseLLM implements ILLM {
  ...
  private async parseError(resp: any): Promise<Error> {
    ...
    if (resp.status === 404 && !resp.url.includes("/v1")) {
      ...
      if (model && resp.url.match("127.0.0.1:11434")) {
        ...
        return new LLMError(text, this, 404); // No need to add HTTP status details
    ...
    // Return LLMError with status code for proper 401 detection in GUI
    return new LLMError(
  ...
  fetch(url: RequestInfo | URL, init?: RequestInit): Promise<Response> {
    ...
    const customFetch = async (input: URL | RequestInfo, init: any) => {
      try {
        // Note: access_token is now handled via completionOptions and requestOptions.headers
        // in the _streamChat method, so we don't need to add it here from localStorage
        ...
        if (e.message.includes("/api/tags")) {
          ...
          if (
            e.code === "ECONNREFUSED" &&
            e.message.includes(CONTINUE_API_URL)
          ) {
  ...
  private async *openAIAdapterStream(
    ...
    for await (const chunk of stream) {
      if (shouldDebugRawChatStream()) {
        console.log("[RAW_CHAT_STREAM_CHUNK][adapter]", JSON.stringify(chunk));
      }
      if (!this.lastRequestId && typeof (chunk as any).id === "string") {
        this.lastRequestId = (chunk as any).id;
      }
      const chatChunk = fromChatCompletionChunk(chunk as any);
      if (shouldDebugRawChatStream() && chatChunk) {
        console.log(
          "[PARSED_CHAT_CHUNK][adapter]",
          JSON.stringify({
            role: chatChunk.role,
            contentPreview:
              typeof chatChunk.content === "string"
                ? chatChunk.content.slice(0, 80)
                : "",
          }),
        );
      }
  ...
  private async *openAIAdapterNonStream(
    ...
    if (shouldDebugRawChatStream()) {
      console.log(
        "[RAW_CHAT_NON_STREAM_RESPONSE][adapter]",
  ...
  async *streamChat(
    ...
    // Add access_token as Authorization Bearer token to requestOptions.headers if provided in options
