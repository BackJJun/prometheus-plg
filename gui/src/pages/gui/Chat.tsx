...
import { loadRemoteModels } from "../../redux/thunks/loadRemoteModels";
import { loadLastSession } from "../../redux/thunks/session";
import { streamResponseThunk } from "../../redux/thunks/streamResponse";
import { isJetBrains, isMetaEquivalentKeyPressed } from "../../util";
import { waitForWorkspacePaths } from "../../util/workspacePaths";
...
// Global flag to track first chat page access after IDE startup
let isFirstChatAccess = true;
...
export function Chat() {
  ...
  useEffect(() => {
    const initializeChatPage = async () => {
      ...
      if (!accessToken) {
        console.log("[Chat] No access token, skipping initialization");
        return;
      }

      if (isFirstChatAccess) {
        // First access: show initializing popup, wait 1s, then load session + models
        console.log("[Chat] First access detected, initializing...");
        setIsInitializing(true);
        isFirstChatAccess = false;

        // Wait for workspacePaths to be set by ParallelListeners (set by IDE)
        // This uses polling instead of fixed delay for reliability
        console.log("[Chat] Waiting for workspacePaths to be set...");
        await waitForWorkspacePaths();
        console.log("[Chat] workspacePaths ready, loading initial data...");

        try {
          console.log("[Chat] Loading last session and models...");
          await dispatch(loadLastSession());
        } catch (e) {
          console.warn("[Chat] Failed to load last session:", e);
        }
        void dispatch(loadRemoteModels());
        setIsInitializing(false);
      } else {
        // Subsequent access: just load models
        console.log("[Chat] Subsequent access, loading models only");
        void dispatch(loadRemoteModels());
      }
    };
  ...
  return (
    <>
      ...
      {/* Initializing overlay on first access */}
      {isInitializing && (
        <div
          style={{
            position: "fixed",
