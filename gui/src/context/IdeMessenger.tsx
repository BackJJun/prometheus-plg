...
export class IdeMessenger implements IIdeMessenger {
  ...
  > {
    ...
    const handler = (event: {
      ...
      if (event.data.messageId === messageId) {
        ...
        if ("error" in responseData) {
          ...
          // Capture statusCode if available
          errorStatusCode = (responseData as any).statusCode;
          return;
    ...
    try {
      while (!done) {
        if (error) {
          // Create error with status property for proper 401 detection
          const err = new Error(error) as Error & { status?: number };
          if (errorStatusCode) {
            err.status = errorStatusCode;
          }
          throw err;
        }
