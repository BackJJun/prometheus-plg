...
interface SubtextContextProvidersContextType {
  ...
  refreshSubmenuProvider: (providerTitle: ContextProviderName) => Promise<void>;
}

const initialContextProviders: SubtextContextProvidersContextType = {
  getSubmenuContextItems: () => [],
  refreshSubmenuProvider: async () => {},
};
...
export const SubmenuContextProvidersProvider = ({
  ...
  const refreshSubmenuProvider = useCallback(
    async (providerTitle: ContextProviderName) => {
      await loadSubmenuItems([providerTitle]);
    },
    [loadSubmenuItems],
  );
