...
export function AssistantOption({
  ...
  const configYamlErrors = useMemo(() => {
    return profile.errors?.filter((error) => {
  ...
  const hasFatalErrors = useMemo(() => {
    return !!configYamlErrors?.find((error) => error.fatal);
  }, [configYamlErrors]);
    ...
    >
      <div className="flex w-full items-center justify-between gap-10 py-0.5">
        <div className="flex w-full items-center gap-3">
          ...
          <span
            className={`line-clamp-1 flex-1 ${selected ? "font-semibold" : ""} ${configYamlErrors && configYamlErrors.length > 0 ? "text-error" : ""}`}
          >
            {profile.title}
          </span>
        </div>
        <div className="flex flex-row items-center gap-1.5">
          ...
          {configYamlErrors && configYamlErrors?.length > 0 && (
