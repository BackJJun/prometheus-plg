...
export function ModeSelect() {
  ...
  const isAgentDisabled = useMemo(() => {
    return (selectedModel as any)?.disableAgentMode === true;
  }, [selectedModel]);
  ...
  const cycleMode = useCallback(() => {
    // If agent mode is disabled, stay in chat
    if (isAgentDisabled) {
      if (mode !== "chat") {
    ...
    // Only cycle between chat and agent
    if (mode === "chat") {
  ...
  }, [mode, mainEditor, isAgentDisabled]);
  ...
  const selectMode = useCallback(
    (newMode: MessageModes) => {
      ...
      // Prevent selecting agent mode if disabled
      if (newMode === "agent" && isAgentDisabled) {
        return;
      }
  ...
  // Auto-switch from agent mode when agent is disabled for this model
  useEffect(() => {
    if (mode === "agent" && isAgentDisabled) {
      dispatch(setMode("chat"));
    }
  }, [mode, isAgentDisabled, dispatch]);
    ...
    <Listbox value={mode} onChange={selectMode}>
      <div className="relative">
        ...
        <ListboxOptions className="min-w-32 max-w-48">
          ...
          >
            <div
              className={`flex flex-row items-center gap-1.5 ${isAgentDisabled ? "opacity-40" : ""}`}
            >
              <ModeIcon mode="agent" />
              ...
              <ToolTip
                ...
                content={
                  isAgentDisabled
                    ? "Agent mode is disabled for this model"
                    : "All tools available"
                }
              >
                <InformationCircleIcon className="h-2.5 w-2.5 flex-shrink-0" />
            ...
            {!isAgentDisabled && !isGoodAtAgentMode && notGreatAtAgent("Agent")}
            <CheckIcon
              className={`ml-auto h-3 w-3 ${mode === "agent" ? "" : "opacity-0"}`}
            />
          </ListboxOption>

          {/* Background mode - commented out */}
          {/* <ListboxOption
            value="background"
            className={"gap-1"}
