...
export function StepContainerPreToolbar({
  ...
  // Get apply model for manual apply operations
  const applyModelTitle = useAppSelector(
    (state) =>
      state.config.config.selectedModelByRole.apply?.title ??
      state.config.config.selectedModelByRole.chat?.title,
  );
  ...
  async function onClickApply() {
    ...
    ideMessenger.post("applyToFile", {
      ...
      applyModelTitle,
      isSearchAndReplace: true, // Use instantApplyDiff to avoid LLM dependency
    });
