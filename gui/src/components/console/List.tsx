...
export default function List({ llmLog, onClickInteraction }: ListProps) {
  ...
  return (
    <ul
      tabIndex={1}
      ref={topRef}
      className="thin-scrollbar group m-0 w-[150px] flex-none list-none overflow-auto border-0 border-r-2 border-solid border-[color:var(--vscode-panel-border)] p-0"
    >
      {llmLog.order.map((id) => (
        <ListItem
          key={id}
