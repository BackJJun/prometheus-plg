  ...
  return (
    <tr
      ...
      onClick={async () => {
        ...
        const currentWorkspacePaths = window.workspacePaths ?? [];
        if (
          sessionMetadata.workspaceDirectory &&
          currentWorkspacePaths.length > 0 &&
          normalizeWorkspaceDirectory(sessionMetadata.workspaceDirectory) !==
            normalizeWorkspaceDirectory(currentWorkspacePaths[0])
        ) {
          const loadLocalSessionInCurrentWindow = async () => {
          ...
          dispatch(
            setDialogMessage(
              <ConfirmationDialog
                ...
                text="This session belongs to a different workspace. Would you like to open it in a new window or the current window?"
                confirmText="New Window"
                cancelText="Current Window"
                onConfirm={() => {
                ...
                onCancel={() => {
                  loadLocalSessionInCurrentWindow();
                }}
              />,
            ),
          );
