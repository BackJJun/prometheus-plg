...
export function getContextProviderDropdownOptions(
  ...
  const items = async ({ query }: { query: string }) => {
    ...
    const referenceContextProvider = availableContextProvidersRef.current?.find(
      (provider) => provider.title === "reference",
    );
    const urlProvider = availableContextProvidersRef.current?.find(
      (provider) => provider.title === "url",
    );
    const referenceProvider: ContextProviderDescription =
      referenceContextProvider ??
        urlProvider ?? {
          title: "reference" as any,
          displayTitle: "Reference",
          description: "Browse and select code references",
          type: "submenu",
        };
    const referenceAliasMatchesQuery =
      query.length === 0 || "reference".startsWith(query.toLowerCase());
    const hasReferenceAlias = contextProviderMatches.some(
      (item) =>
        item.id === referenceProvider.title && item.title === "Reference",
    );
    if (referenceAliasMatchesQuery && !hasReferenceAlias) {
      contextProviderMatches.push({
        description: referenceProvider.description,
        id: referenceProvider.title,
        title: "Reference",
        label: "Reference",
        type: "contextProvider",
        contextProvider: referenceProvider,
      });
    }

    if (contextProviderMatches.length) {
      // "Add more context providers" 메뉴 항목 숨김 처리
      // contextProviderMatches.push({
