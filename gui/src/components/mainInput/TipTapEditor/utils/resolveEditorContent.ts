...
async function gatherContextItems({
  ...
  console.log(
    "[resolveEditorContent] Processing context requests:",
    deduplicatedInputs,
  );
  for (const item of deduplicatedInputs) {
    if (item.resolvedContextItem) {
      contextItems.push(item.resolvedContextItem);
      continue;
    }

    console.log(
      "[resolveEditorContent] Requesting context item:",
      item.provider,
      item.query,
    );
    ...
    console.log(
      "[resolveEditorContent] Context item result:",
      result.status,
      result.status === "success" ? result.content?.length : "N/A",
    );
    if (result.status === "success") {
      contextItems.push(...result.content);
    } else {
      console.error(
        "[resolveEditorContent] Failed to get context items:",
        result,
      );
    }
  }
