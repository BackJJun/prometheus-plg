...
export function BlockSettingsTopToolbar() {
  ...
  const { selectedProfile, isLoginDialogOpen } = useAuth();
  ...
  // Distinguish between fatal errors and warnings (only for config.yaml errors)
  const hasFatalError =
    configYamlErrors && configYamlErrors.some((error) => error.fatal);
  const hasWarning =
    configYamlErrors && configYamlErrors.some((error) => !error.fatal);
  const shouldShowError = hasFatalError || hasWarning;
  ...
  return (
    <div className="flex flex-1 items-center justify-between gap-3">
      <div className="flex items-center gap-1">
        ...
        {!hasActiveContent && !isLoginDialogOpen && (
