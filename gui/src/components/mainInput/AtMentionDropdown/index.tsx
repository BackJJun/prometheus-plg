...
const ItemsDiv = styled.div.attrs({
...
const AtMentionDropdown = forwardRef((props: AtMentionDropdownProps, ref) => {
  ...
  const [activeSubmenuProviderId, setActiveSubmenuProviderId] = useState<
    string | undefined
  >(undefined);
  ...
  const submenuIsResolving =
    !!activeSubmenuProviderId &&
    !querySubmenuItem &&
    props.items.length > 0 &&
    props.items.every((item) => item.type === "contextProvider");
  ...
  }, [subMenuTitle, props.items, props.editor, submenuIsResolving]);

  useEffect(() => {
    if (
      activeSubmenuProviderId &&
      props.items.some((item) => item.type !== "contextProvider")
    ) {
      return;
    }

    if (!subMenuTitle) {
      setActiveSubmenuProviderId(undefined);
    }
  }, [activeSubmenuProviderId, props.items, subMenuTitle]);
  ...
  const selectItem = (index: number) => {
    ...
    if (
      ...
      setActiveSubmenuProviderId(item.id);
