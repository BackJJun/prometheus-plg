...
const LayoutContent = () => {
  ...
  const showDialogCloseButton = useAppSelector(
    (state) => state.ui.showDialogCloseButton,
  );
  ...
  useWebviewListener(
    "getSecurityCheckMode",
    async () => {
      return securityCheckMode;
    },
    [securityCheckMode],
  );
  ...
  useWebviewListener(
    "getSecurityDisplayMode",
    async () => {
      return securityDisplayMode;
    },
    [securityDisplayMode],
  );
  ...
  // Show loading or hide content until auth is ready and user is logged in
  const shouldShowContent = isAuthReady && session;
