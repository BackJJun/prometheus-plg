  ...
  if (toolCallDelta.id) {
    ...
    if (existingStateIndex === -1) {
      for (const state of toolCallStates) {
        ...
        try {
          JSON.parse(state.toolCall.function.arguments);
          if (state.toolCall.function.name) {
            state.status = "generated";
          }
        } catch {
          // Keep the previous call in generating state if its arguments are not complete yet.
        }
      }
    }
  } else {
...
type SessionState = {
  ...
  hasAuthError?: boolean; // Track authentication errors to prevent saving
};
...
export const sessionSlice = createSlice({
  ...
  reducers: {
    ...
    /**
     * GPT-OSS Tool Call 재시도를 위해 마지막 assistant/thinking 메시지들 삭제
     * 마지막 user 또는 tool 메시지 이후의 모든 메시지를 제거합니다.
     */
    removeLastAssistantMessages: (state) => {
      while (state.history.length > 0) {
        const last = state.history[state.history.length - 1];
        if (last.message.role === "user" || last.message.role === "tool") {
          break;
        }
        state.history.pop();
      }
