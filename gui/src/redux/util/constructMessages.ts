  ...
  for (const item of historyCopy) {
    ...
    if (item.message.role === "user") {
      ...
      const ctxItemParts = item.contextItems
        .map((ctxItem) => {
          const providerTitle = ctxItem.id?.providerTitle?.toLowerCase();
          const isReference = providerTitle === "reference";
          const referenceDocId = isReference
            ? ctxItem.uri?.value || ctxItem.id?.itemId
            : undefined;
          const referenceHeader = isReference
            ? `[REFERENCE_SELECTED id=${referenceDocId ?? ""} name=${ctxItem.name ?? ""}]\n[REFERENCE_INSTRUCTION tool=read_reference doc_id=${referenceDocId ?? ""}]\n`
            : "";
          const contextContent = isReference ? "" : `${ctxItem.content}\n`;
          return {
            type: "text",
            text: `${referenceHeader}${contextContent}`,
          } as TextMessagePart;
        })
      ...
      // Thinking is a UI/history-only role and must not be sent back to chat APIs.
      continue;
