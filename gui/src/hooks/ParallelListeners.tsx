...
function ParallelListeners() {
  ...
  useEffect(() => {
    async function initialLoadConfig() {
      ...
      // Load models from API after config is loaded
      const accessToken = localStorage.getItem("access_token");
      if (accessToken) {
        console.log("[ParallelListeners] Loading models from API");
      ...
      if (!isJetBrains()) {
        ...
        if (result.status === "success") {
          (window as any).workspacePaths = result.content;
        }
        // Removed loadLastSession() - user should explicitly click "Last Session" button
      }
    }
    ...
    const interval = setInterval(() => {
      if (hasDoneInitialConfigLoad.current) {
        ...
        // Removed refreshSessionMetadata() - only load when History page is opened
    ...
    if (jetbrains) {
      ...
      void ideMessenger
        ...
        .then((result) => {
          ...
          // Load models after workspacePaths is set (for JetBrains)
          // This ensures /api/models is called reliably on IDE restart
          const accessToken = localStorage.getItem("access_token");
          if (accessToken) {
            console.log(
              "[ParallelListeners] JetBrains: Loading models after onLoad",
            );
            void dispatch(loadRemoteModels());
          }
