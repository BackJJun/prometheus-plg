...
async function buildWithEsbuild() {
  ...
  let continueApiUrl = "http://localhost:8000";
  let defaultServerApiUrl = "http://localhost:8000";
  try {
    if (fs.existsSync(envPath)) {
      const envContent = fs.readFileSync(envPath, "utf8");
      const continueMatch = envContent.match(/CONTINUE_API_URL=(.*)/);
      if (continueMatch) {
        continueApiUrl = continueMatch[1].trim();
        console.log(
          `[info] Loaded CONTINUE_API_URL from .env: ${continueApiUrl}`,
        );
      }
      const defaultMatch = envContent.match(/DEFAULT_SERVER_API_URL=(.*)/);
      if (defaultMatch) {
        defaultServerApiUrl = defaultMatch[1].trim();
        console.log(
          `[info] Loaded DEFAULT_SERVER_API_URL from .env: ${defaultServerApiUrl}`,
        );
      }
    }
  } catch (e) {
    console.warn("[warn] Failed to read .env file:", e);
  }
  ...
  await esbuild.build({
    ...
    define: {
      ...
      "process.env.CONTINUE_API_URL": JSON.stringify(continueApiUrl),
      "process.env.DEFAULT_SERVER_API_URL": JSON.stringify(defaultServerApiUrl),
    },
  });
}
...
(async () => {
  ...
  fs.mkdirSync(treeSitterWasmsDir, { recursive: true });
  ...
  fs.mkdirSync(treeSitterDir, { recursive: true });
  await new Promise((resolve, reject) => {
    ncp(
