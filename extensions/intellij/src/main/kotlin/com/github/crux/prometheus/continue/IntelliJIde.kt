...
import com.github.crux.prometheus.constants.ContinueConstants
import com.github.crux.prometheus.constants.getContinueGlobalPath
import com.github.crux.prometheus.`continue`.file.FileUtils
...
import com.github.crux.prometheus.utils.*
...
class IntelliJIDE(
                ...
                toolWindow?.activate({
                    try {
                        ...
                        if (terminalOptions.reuseTerminal == true && terminalView.getWidgets().isNotEmpty()) {
                            if (terminalOptions.terminalName != null) {
                                widget = terminalView.getWidgets().filterIsInstance<ShellTerminalWidget>()
                                    .firstOrNull {
                                        contentManager.getContent(it)?.tabName == terminalOptions.terminalName
                                                && !it.hasRunningCommands()
                                    }
                            } else {
                                widget = terminalView.getWidgets().filterIsInstance<ShellTerminalWidget>()
                                    .firstOrNull { contentManager.getContent(it)?.isSelected ?: false }
                        ...
                        if (widget == null) {
                            ...
                            val content = widget?.let { toolWindow.contentManager.getContent(it) }
                            if (content != null) {
    ...
    override suspend fun showToast(type: ToastType, message: String, vararg otherParams: Any): Any {
        return withContext(Dispatchers.Default) {
            ...
            val notification = NotificationGroupManager.getInstance().getNotificationGroup("Prometheus")
