package com.github.crux.prometheus.`continue`

import com.github.crux.prometheus.*
import com.github.crux.prometheus.activities.ContinuePluginDisposable
import com.github.crux.prometheus.activities.showTutorial
...
import com.github.crux.prometheus.editor.DiffStreamService
import com.github.crux.prometheus.editor.EditorUtils
import com.github.crux.prometheus.error.ContinueSentryService
import com.github.crux.prometheus.protocol.*
...
import com.github.crux.prometheus.utils.getMachineUniqueID
    ...
    fun handleMessage(msg: String, respond: (Any?) -> Unit) {
        coroutineScope.launch(limitedDispatcher) {
            ...
            try {
                when (messageType) {
                    "workspace/open" -> {
                        val workspacePath = dataElement.asString
                        ApplicationManager.getApplication().invokeLater {
                            val path = if (workspacePath.startsWith("file:")) {
                                java.nio.file.Paths.get(java.net.URI.create(workspacePath))
                            } else {
                                java.nio.file.Paths.get(workspacePath)
                            }
                            com.intellij.ide.impl.ProjectUtil.openOrImport(
                                path,
                                com.intellij.ide.impl.OpenProjectTask(
                    ...
                    "showFile" -> {
                        ...
                        println("[Prometheus][showFile][IntelliJ] filepath: ${params.filepath}")
                    ...
                    "closeSidebar" -> {
                        ApplicationManager.getApplication().invokeLater {
                            ...
                            val toolWindow = toolWindowManager.getToolWindow("Prometheus")
                            toolWindow?.hide()
                        }
                    }
