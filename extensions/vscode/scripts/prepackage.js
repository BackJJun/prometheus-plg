...
void (async () => {
  ...
  // 기존에 index.html 파일이 존재하는 경우에만 임시 백업(tmp_index.html)을 수행합니다.
  // 최초 빌드 시에는 파일이 없어서 발생하는 복사 에러(ENOENT)를 방지하는 예외 처리입니다.
  if (hasIndexHtml) {
  ...
  // Put back index.html
  // 만약 사전에 백업해 둔 tmp_index.html이 있었다면, 그것을 다시 index.html로 복구합니다.
  if (hasIndexHtml) {
    ...
    // 기존 index.html이 없었던 경우, gui/dist에서 생성된 jetbrains_index.html을 활용하여 index.html을 만들어줍니다.
    const jetbrainsIndex = path.join(
      intellijExtensionWebviewPath,
      "jetbrains_index.html",
    );
    if (fs.existsSync(jetbrainsIndex)) {
      fs.copyFileSync(jetbrainsIndex, indexHtmlPath);
    }
  }
