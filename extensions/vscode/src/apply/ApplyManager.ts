...
import { VsCodeRuntimeClientAdapter } from "../runtime";
...
export class ApplyManager {
  private runtimeAdapter: VsCodeRuntimeClientAdapter;
  ...
  constructor(
    ...
    this.runtimeAdapter = new VsCodeRuntimeClientAdapter(webviewProtocol);
  }
  ...
  private async handleExistingDocument(
    ...
    // Try to find model by title if provided (from GUI)
    let llm = null;
    if (applyModelTitle) {
      const allModels = [
      ...
      llm = allModels.find((m) => m.title === applyModelTitle) ?? null;
    }

    // Fallback to config's selected model if not found by title
    if (!llm) {
      llm = config.selectedModelByRole.apply ?? config.selectedModelByRole.chat;
    }
  ...
  private reportApplyState(payload: {
    streamId: string;
    status: string;
    toolCallId?: string;
    fileContent?: string;
    ...
    createRuntime({
      ...
      getToolCallId: () => payload.toolCallId ?? payload.streamId,
      getToolCallName: () => "apply",
      getProcessedArgs: () => ({}),
    }).reportApplyState(payload);
  }
}
