...
export function getSessionPersistenceSnapshot(
  ...
  return {
    ...modifySessionBeforeSave(session),
...
function toRuntimePermissionRequest(pendingPermission: any) {
  if (!pendingPermission?.requestId || !pendingPermission?.toolName) {
    return undefined;
  }
  ...
  return {
    ...
    toolCallId:
      ...
      pendingPermission.requestId.replace(/^runtime-/, ""),
    toolName: pendingPermission.toolName,
    arguments: pendingPermission.toolArgs ?? {},
    preview: pendingPermission.toolCallPreview,
  };
}

function withRuntimeStateSnapshot(
  session: PersistedSession,
  isProcessing: boolean,
  pendingPermission: any,
): PersistedSession {
  const previousRuntimeState = session.runtimeState;
  const nextRuntimeState: NonNullable<Session["runtimeState"]> = {
    sessionId: session.sessionId,
    title: session.title,
    workspaceDirectory: session.workspaceDirectory,
    history: session.history,
    toolStates: previousRuntimeState?.toolStates ?? {},
    pendingPermission:
      toRuntimePermissionRequest(pendingPermission) ??
      previousRuntimeState?.pendingPermission,
    isProcessing,
    currentTurnId: previousRuntimeState?.currentTurnId,
  };

  if (!isProcessing && !pendingPermission) {
    nextRuntimeState.pendingPermission = undefined;
  }
...
export function getCompleteStateSnapshot(
  ...
  const sessionWithRuntimeState = withRuntimeStateSnapshot(
    session,
    isProcessing,
    pendingPermission,
  );
  return {
    session: getSessionPersistenceSnapshot(sessionWithRuntimeState),
    isProcessing,
    messageQueueLength,
    pendingPermission,
  };
 ...
 */
export function saveSession(runtimeState?: Session["runtimeState"]): void {
  try {
...
export function loadSession(): PersistedSession | null {
  try {
    ...
    const session: PersistedSession = JSON.parse(
...
function getSessionMetadataWithPreview(
  ...
  try {
    const sessionData: PersistedSession = JSON.parse(
 ...
 */
export function getCurrentSession(): PersistedSession {
